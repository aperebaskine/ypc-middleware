...
public class HibernateUtils {
	...
	static {
		try {
			...
			logger.fatal("Failed to initialize Hibernate. Stack trace: ", t);
	...
	public static <T extends AbstractEntity<?>> String getTableName(Class<T> entityClass) {
		SessionFactoryImplementor factory = sessionFactory.unwrap(SessionFactoryImplementor.class);
		RuntimeMetamodelsImplementor runtimeMetamodels = factory.getRuntimeMetamodels();
		EntityMappingType mappingType = runtimeMetamodels.getEntityMappingType(entityClass);
		return mappingType.getIdentifierTableDetails().getTableName();		
	}
